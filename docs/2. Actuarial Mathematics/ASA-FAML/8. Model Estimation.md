...
Censored and Truncated data can only be worked with using dedicated methods.
...
!!! Note
    ...
    The results for the two methods are not that far off from one another. This can be used as a sense check if asked to calculate both.
