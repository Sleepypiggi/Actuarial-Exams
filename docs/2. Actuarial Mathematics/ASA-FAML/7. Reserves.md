...
On the other hand, expense premiums are typically **higher than the renewal expenses**, resulting in a negative loss in all subsequent years, recovering the initial loss over time.

At time 1, all future expense premiums will be larger than the corresponding renewal expenses, resulting in a **negative expense reserve** from that point onwards:

$$
\begin{aligned}
    \text{Expense Premium} &\gt \text{Renewal Expense} \\
    \text{EPV(Expense Premium)} &\gt \text{EPV(Renewal Expense)} \\
    {}_{t}V^{\text{Expense}} < 0, t \gt 0
\end{aligned}
$$
...
However, net premium reserves tend to be **too high** relative to the more accurate gross premium reserves.
