...
Review: Increasing Annuities Certain Due
...
$$
\begin{aligned}
    \text{EPV Arith WL}_\text{Due}
    &= \sum^{\infty}_{k=0} (k+1) v^k {}_{k}p_x \\
    (I\ddot{a})_x
    &= v^0 {}_{0}p_x + \sum^{\infty}_{k=1} (k+1) v^k {}_{k}p_x \\
    &= 1 + \sum^{\infty}_{k=0} (k+2) v^{k+1} {}_{k+1}p_x
\end{aligned}
$$

$$
\begin{aligned}
    \text{EPV Arith TA}_\text{Due}
