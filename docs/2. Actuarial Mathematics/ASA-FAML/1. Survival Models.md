...
**Life Expectancy** measures the number of years a person is *expected* to live. It is calculated as the **Expectation** of the distribution of future lifetime:
...
The expectation of the survival distribution is also commonly denoted as $\mathring{e}_x$.
...
There is a clear positive relationship between age and mortality - mortality **increases exponentially** with age.
...
Makeham added a **constant $c$** into the Gompertz model, resulting in the **Makeham Gompertz Model**:

$$
    \mu_{x} = A + Bc^{x}
$$

$c$ represents the **age independent mortality** - dying from reasons unrelated to age, such as from accidents or natural catastrophes.
