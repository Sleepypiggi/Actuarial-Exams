...
\begin{aligned}
    ...
    & \approx h[f(a) + f(a+2h) + \dots + f(b-h)] - \frac{h}{2} [f(a)-f(b)]
\end{aligned}
$$

> The proof relies on the fact that other than $f(a)$ and $f(b)$, all other terms are repeated twice.
>
> Note that last and second last step are opposites of each other - the second last one excluded $f(a)$ from the main summation, which is why it is added back and subtracts $f(b)$, while the last one excludes $f(b)$ instead.

<!-- Trapezium Rule -->
![Trapezium Rule](Assets/1.%20Survival%20Models.md/Trapezium%20Rule.png){.center}

Thus, the trapezium rule can be applied to the survival function, assuming $h=1$:
...
> Since the survival function is decreasing, the curve passes through the top right of the rectangles, thus the sum of the rectangles are properly known as the **Right Riemann Sum**.
...
Thus, the trapezoidal approximation can be made more precise:
