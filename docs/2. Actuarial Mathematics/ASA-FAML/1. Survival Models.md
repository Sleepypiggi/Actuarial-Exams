...
It is important to understand that this is calculated from the perspective of age $x$, despite the actual death occuring much later.
...
As a discrete distribution, it uses a Probability *Mass* Function rather than a Density Function. Thus, the intepretation is slightly different.

$P(K_x = k)$ represents the probability
...
The probability of death/survival can be calculated as the **proportion of people who died/survived** in that period:
...
Update the age for suvival model
curtate Kx = 5 means die within 5 (inclusive) and 6 (exlusive)