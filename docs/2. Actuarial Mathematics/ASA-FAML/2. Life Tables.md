...
$$
    \ell_{x+s} = (1-s) \cdot \ell_{x} + t \cdot \ell_{x} \\
$$
...
!!! Tip
    ...
    This result should not be surprising, as it is an extension of the previous result on the exponential survival functions.

    Since the probabilities are **memoryless**, age does not matter:
