...
Deferred Assurances are variations of any of the above assurances, where the assurance **starts $k$ years later rather** than immediately.
...
\begin{aligned}
    E(\text{DWL})
    &= 0 \cdot {}_{n}p_{x} + \sum^{\infty}_{k = n} v^{K + 1} \cdot {}_{k|}q_{x} \\
    {}_{n|} A_{x}
...
Questions could also ask for the **probability that the present value (NOT THE EPV!) of the benefits takes on some value** $u$.
...
The RHS is unlikely to be an integer. However, $K_x$ can only take integer values. Thus, the values are **rounded up** to the nearest whole number (EG. 5):
...
Questions will **not be as straightforward as above**. Most of the time, the required value of $K_x$ has to be determined **intuitively from the given scenario**.
...
Since premiums are only received when the policyholder is alive, the insurer must collect **at least 10 rounds of premium** to "break even". In other words, the question is asking for the **probability that $K_x \gt 9$**.

!!! Warning

    It is a common mistake to use $K_x \gt 10$, since 10 payments must be made.

    However, since premiums are paid at the **start of the year**, it doesnt matter whether the policyholder survives the 10th year or not; as long as they survive 9 years to see the 10th year.

#### **Percentiles**

Similarly, questions could ask for the **percentile of the PV**. Recall that the definition of a percentile is the value in which $100p$ of **observations fall BELOW**.

Since the PV is a transformation of the future lifetime variable, the **percentile of the PV is the percentile of the future lifetime variable**.

Let $Z_{p}$ be the percentile of the PV and $K_{p}$ be the percentile of the future lifetime:

$$
    Z_{p} = B \cdot V^{K_{p}+1}
$$

Since the PV is a **decreasing function**, the range of future lifetime that corresponds to $Z_{p}$ is always from **$K_{p}$ to its upper limit**:

<!-- Obtained from Coaching Actuaries -->
![Percentile](Assets/3.%20Life%20Assurances.md/Percentile.png){.center}

This can be extremely **hard to grasp** because we learnt percentiles with increasing RVs, which may lead us to mistakenly believe that percentiles are always calculated using the inverse CDF.

$$
\begin{aligned}
    P(v^{K_x+1} \le Z_{p}) \le p \\
    P(K_x \gt K_{p}) \le p \\
    {}_{K_{p}+1}p_{x} \le p \\
    \frac{\ell_{x + K_{p} + 1}}{\ell_{x}} \le p \\
    \ell_{x + K_{p} + 1} \le p \cdot \ell_{x}
\end{aligned}
$$

We can then search the SULT for the **smallest integer age** where the inequality is fulfilled and then solve for $K_{p}$ and thus $Z_{p}$.

!!! Note

    If the future lifetime variable is **continuous**, then the **percentile must be more precise**; using a fractional age instead.

    Thus, the fractional age must be interpolated between $\ell_{x + K_{p} + 1}$ and $\ell_{x}$ using an appropriate assumption.
