...
> Since WLs are just term assurances with infinite coverage, it can be extended to WLs as well.
...
$$
    (\bar{I}\bar{A}) = \int^{\infty}_0 t v^t \cdot {}_{t}p_x \cdot \mu_{x+t}
$$
...
$$
\begin{aligned}
    (\bar{I}\bar{A}) &= (I\bar{A}) - \frac{1}{2} \cdot 1 \cdot 1 \cdot \bar{A}_x \\
