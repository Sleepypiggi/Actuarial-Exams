...
The BF method can also be calculated via a **weighted average of the two methods**:
...
!!! Warning
    ...
    The above applies for the OVERALL RESERVE, not the Case or IBNR reserves.

As the claims approach maturity, more the LDF factors decrease, causing the **weights to the CL method** to increase. This is intuitive, as **more data is available** for those claims, thus **more weight is given to the method that uses actual claims experience** (CL).

Naturally, this means that the BF reserve is **between the reserves calculated** via the two methods:

$$
\begin{aligned}
    \text{ELR Reserve} \lt \text{BF Reserve} \lt \text{CL Reserve} \\
    \text{ELR Reserve} \gt \text{BF Reserve} \gt \text{CL Reserve}
\end{aligned}
$$

!!! Warning

    Despite the above relationship, there is **NO relationship** between the ELR and CL reserves - either could be larger than the other.
