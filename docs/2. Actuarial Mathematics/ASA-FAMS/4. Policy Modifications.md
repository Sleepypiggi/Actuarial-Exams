...
Imposing a deductibles will always **decrease the number of payments**, resulting a **adjusted frequency distribution** $(N^{*})$.
...
\begin{aligned}
    ...
    &=
    \begin{cases}
        1 (\text{Success}),& X \gt d \\
        0 (\text{Failure}),& X \le d
    \end{cases} \\
    \\
    v &= P(X \gt d) \\
    P_I(t) &= 1 + v(t-1)
...
\begin{aligned}
   ...
   \therefore N^{*} &\sim \text{Binomial} (n, pv) \\
   \\
