...
In terms of MLE, this means that the **distribution is conditioned** on having values above $d$ or below $u$. Thus, the likelihood functions should be constructed using conditional PMFs/PDFs instead:
...
In terms of MLE, this means that the **likelihoods of certain observations are incorrect**. For censored observations, the likelihoods should be represented by the **probabilities are larger/smaller than the threshold**, rather than equal to their observation.

Thus, for censored observations at the threshold, the updated likelihoods must be used:

$$
\begin{aligned}
    L^{LC}(\theta) &= \prod P_{\theta} (X = x_i) \cdot \prod P(X \le d) \\
    L^{RC}(\theta) &= \prod P_{\theta} (X = x_i) \cdot \prod P(X \ge u)
\end{aligned}
$$

If an observation is both left truncated and right censored, then its likelihood function is:
