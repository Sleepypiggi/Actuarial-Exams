...
!!! Warning
    ...
    The above example assumed that **rates are in effect for one year** (CY), exactly at the start and end of the PY. In practice, rates can **start and end at any time**.
...
Remember that this is the **final step** for ratemaking. This means that **all other steps must be completed** before this. Thus, if the question provides **non-trended or non-aggregated** data, then it must be done before continuing.
...
!!! Warning
    ...
    It is a common mistake to use the weighted average of the indicative premiums or use the weighted average of the raw losses or premiums. Be careful!
