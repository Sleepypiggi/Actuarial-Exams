...
Most questions *already* provide the aggregated losses. However, if **given raw data**, we are expected to know how to aggregate them ourselves.
...
The calculation for the duration of each period depends on what was used to aggregate the loss data:

* **AY Aggregation**: Period is the AY
* **PY Aggregation**: Period is the PY plus the policy term

Assume that rates are in effect for **exactly one year**, from the start to the end of the PY. Assuming that policies are **written uniformly throughout the year**, it means that there will be policies **written on the first and last day of the PY**:
...
$$
    \text{PY Timespan} = \text{Rates Duration} + \text{Policy Term}
$$
...
!!! Warning
    ...
    The above example assumed that rates are in effect for one year, exactly at the start and end of the PY. In practice, rates can **start and end at any time**.

The trend period is thus the difference between the **midpoint** of the two periods:
...
Similar to losses, historical premiums must be first aggregated and then **brought to their current level** ("trended").
...
As with loss aggregation, most questions already provide the aggregated premiums. However, if given raw data, we are expected to know how to aggregate them ourselves.
...
Instead of being provided with the exact rates, yearly aggregate **rate changes** are provided:
...
!!! Note
    ...
    \begin{aligned}
        \text{CY 2014 Rates} &= 1 \\
        \text{CY 2015 Rates} &= 1 \cdot (1.05) \\
        \text{CY 2016 Rates} &= 1 \cdot (1.05)(1.1) \\
        \text{CY 2017 Rates} &= 1 \cdot (1.05)(1.1)(0.98) \\
    \end{aligned}
    $$
...
The sufficient amount of premium is known as the **Indicative Premium** $(P^I)$, which can be solved for using the fundamental insurance equation:
...
\begin{aligned}
    ...
    \\
    \therefore \text{Indicative Rate} &= \frac{\frac{L}{n^e} + \frac{\text{FE}}{P^c}}{\text{PLR}}
\end{aligned}
$$

#### **Practical Tips**

Although both methods are equivalent, they are NOT expected to produce the similar results.

For both methods, **always trend the losses and premiums** before plugging them into the equations.

If multiple years of data are given, then use the **weighted average** of the loss ratios or pure premiums. If any, the question **will usually specify the weights** to use.

!!! Warning

    It is the weighted average of the Loss Ratios and Pure Premiums, NOT the actual losses and premiums.
